...
export default function FormDialog() {
  ...
  const signup = async (e) => {
    ...
    try {
        const response = await axios.post('http://localhost:8000/signup', { username, email, password }, { withCredentials: true });
        const { data } = response;
  ...
  const handlelogin = async (e) => {
    ...
    try {
        const response = await axios.post('http://localhost:8000/login', { email:email1, password:password1 }, { withCredentials: true });
        const { data } = response;
        setUser(data.username);
        setId(data.id);
        console.log(data);
